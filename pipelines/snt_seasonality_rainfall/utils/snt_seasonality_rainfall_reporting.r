...
rainfall_month_palette <- function() {
    c(
        "January" = "#9E0142",
        "February" = "#D53E4F",
        "March" = "#F46D43",
...
make_rainfall_start_month_plot <- function(
    ...
    month_labels <- rainfall_month_labels_en()
    month_colors <- rainfall_month_palette()
...
make_rainfall_proportion_plot <- function(
    ...
    ggplot2::ggplot(plot_data) +
        ...
        ggplot2::theme_void() +
        ggplot2::labs(
            title = "Percentage of annual rainfall falling within the seasonal block",
            subtitle = subtitle_text,
            caption = paste("Data source:", data_source),
            fill = NULL
        ) +
