...
plot_outliers_by_district_facet_year <- function(ind_name, df, outlier_col) {
    ...
    ggplot2::ggplot(df_ind, ggplot2::aes(x = ADM2_ID, y = VALUE)) +
        ...
        ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 75, hjust = 1, size = 7))
}
...
plot_coherence_heatmap <- function(df, selected_year, agg_level = "ADM1_NAME", filename = NULL, do_plot = TRUE) {
    ...
    df_year <- df %>%
        ...
        dplyr::summarise(pct_coherent = mean(pct_coherent, na.rm = TRUE), .groups = "drop") %>%
    ...
    agg_label <- if (agg_level == "ADM1_NAME") "niveau administratif 1" else if (agg_level == "ADM2_NAME") "niveau administratif 2" else agg_level
    p <- ggplot2::ggplot(df_year, ggplot2::aes(x = check_label, y = .data[[agg_level]], fill = pct_coherent)) +
        ggplot2::geom_tile(color = "white", linewidth = 0.2) +
        ggplot2::geom_text(ggplot2::aes(label = sprintf("%.0f%%", pct_coherent)), size = 5, fontface = "bold", color = "white") +
        viridis::scale_fill_viridis(name = "% cohérent", limits = c(0, 100), option = "viridis", direction = -1) +
        ggplot2::labs(title = paste0("Cohérence des données par ", agg_label, " - ", selected_year), x = "Règle de cohérence", y = agg_label) +
        ggplot2::theme_minimal(base_size = 14) +
        ggplot2::theme(
    ...
    if (!is.null(filename)) {
        ggplot2::ggsave(filename = filename, plot = p, width = 14, height = plot_height, dpi = 300, limitsize = FALSE)
    }
    if (do_plot) {
        print(p)
    }
