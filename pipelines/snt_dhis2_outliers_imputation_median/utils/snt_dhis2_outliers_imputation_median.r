...
#' replaced by the ceiling of a centered rolling median, preserving non-outlier
#' observations. Windows holding no usable value yield NA rather than NaN.
#'
...
#' @param n Integer. Width of the centered rolling window. Defaults to 3.
#' @return Data frame with a VALUE_IMPUTED column and the helper columns removed.
#'
#' @export
impute_outliers_dt <- function(dt, outlier_col, n = 3) {
    ...
    dt[, MEDIAN_IMPUTED := as.numeric(data.table::frollapply(
        TO_IMPUTE,
        N = n,
        FUN = function(x) {
